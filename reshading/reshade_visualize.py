...
from taskonomy_dataset import TaskonomyDatasetReshade
...
num_classes = 1
ckpt = './checkpoints/reshade-ckpt.pth'
best = './checkpoints/reshade-best.pth'

def main():
    taskonomy_transform = transforms.Compose([transforms.ToTensor(),
                                              transforms.Normalize((0.5456, 0.5176, 0.4863),
                                                                   (0.1825, 0.1965, 0.2172))])
    taskonomy_testset = TaskonomyDatasetReshade('../data/tiny_taskonomy_rgb_test.csv',
                                                '/reshading',
                                                'reshading.png',
                                                resize256 = True,
    ...
    for i in range(args.test_batch):
        ...
        label = labels[i]
        plt.imshow(label, cmap=plt.get_cmap('gray'), vmin=0, vmax=1)
        plt.colorbar()
        plt.axis('off')

        # Predicted depth
        plt.subplot(3,args.test_batch,i+2*args.test_batch+1)
        plt.imshow(pred[i], cmap=plt.get_cmap('gray'), vmin=0, vmax=1)
        plt.colorbar()
        plt.axis('off')
