...
def main():
    ...
    for i in range(args.test_batch):
        ...
        label = labels[i]
        label = np.transpose(label, (1,2,0)) # (3,256,256) --> (256,256,3)
        plt.imshow(label, cmap=plt.get_cmap('hot'), vmin=-1, vmax=1)
        #plt.colorbar()
        plt.axis('off')
        ...
        plt.imshow(p, cmap=plt.get_cmap('hot'), vmin=-1, vmax=1)
        #plt.colorbar()
        plt.axis('off')
