...
parser.add_argument('--source', default='depth', type=str, choices=[
                                                                    'autoencoder',
                                                                    'class-object',
                                                                    'class-place',
                                                                    'inpainting',
...
def check_source_task():
    if args.source == 'autoencoder':
        ...
        source_decoder = decoder256x256
    elif args.source == 'inpainting':
        source_num_classes = 3
        source = '../inpainting/checkpoints/inpaint-best.pth'
        source_decoder = decoder256x256
    elif args.source == 'normal':
        source_num_classes = 3
        source = '../normal/checkpoints/normal-best.pth'
        source_decoder = decoder256x256
    elif args.source == 'depth':
        source_num_classes = 1
        source = '../rgb2depth/checkpoints/rgb2depth-best.pth'
        source_decoder = decoder256x256
    elif args.source == 'euclidean':
        source_num_classes = 1
        source = '../rgb2mist/checkpoints/rgb2mist-best.pth'
        source_decoder = decoder256x256
    elif args.source == 'class-object':
        source_num_classes = 1000
        source = '../class-object/checkpoints/class_1000-best.pth'
        source_decoder = decoder1000
    elif args.source == 'semantic':
        source_num_classes = 17
        source = '../semantic-segment/checkpoints/semantic-segment-best.pth'
        source_decoder = decoder256x256
    elif args.source == 'vanishing-point':
        source_num_classes = 9
        source = '../vanishing-point/checkpoints/Vanish-best.pth'
        source_decoder = decoder1000
    return source_num_classes, source, source_decoder

num_classes = 1
if os.path.isdir('./checkpoints') == False:
    os.makedirs('./checkpoints')
ckpt = './checkpoints/rgb2depth-transfer-ckpt.pth'
best = './checkpoints/rgb2depth-transfer-best.pth'
...
def main():
    ...
    taskonomy_trainset = TaskonomyDatasetZbuffer('../data/tiny_taskonomy_rgb_val.csv',
                                                 '/depth_zbuffer',
                                                 'depth_zbuffer.png',
                                                 resize256 = True,
    ...
    if args.resume:
        ...
        if args.source == 'depth':
            print('[INFO] Encoder Decoder with random initialization.')
        else:
            print('[INFO] Copying weights for *Encoder* of model from pre-trained source task model.')
            model.encoder.load_state_dict(source_model.encoder.state_dict())
            print('[INFO] Freezing weights for *Encoder* of model.')
